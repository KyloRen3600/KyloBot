...
def download_update():
	...
	zip_ref.close()
	path = './Temp/KyloBot-master/Code'
	dst = "./"
	files = os.listdir(path)
	for file in files:
		try:
			copyfile('./Temp/KyloBot-master/Code/{0}'.format(file), "{1}".format(dst, file))
			os.remove('./Temp/KyloBot-master/Code/{0}'.format(file))
		except:
			copy_tree('./Temp/KyloBot-master/Code/{0}'.format(file), "{1}".format(dst, file))
			rmtree('./Temp/KyloBot-master/Code/{0}'.format(file))
